...
function migrateGlossaryCatalog() {
  ...
  for (const term of GLOSSARY_CATALOG) {
    if (!existing.has(normalizeGlossaryName(term.name))) {
      state.glossary.push({ ...term, id: makeGlossaryId() });
...
function termCardHtml(term, map) {
  return `<article class="glossary-item" id="term-${escapeHtml(term.id)}">
    <div class="glossary-item-header">
      <div><h3 class="glossary-item-title">${escapeHtml(term.name)}</h3><span class="glossary-badge">${escapeHtml(term.category)}</span></div>
...
function renderGlossary() {
  ...
  if (!terms.length) {
    ...
    glossaryElement.glossaryList.innerHTML = [...groups.entries()].map(([category,items]) => {
      const open = keywordActive || Boolean(categoryFilter) || items.length < GLOSSARY_PREVIEW_LIMIT;
...
function renderRecentGlossary() {
  const recent = [...state.glossary]
    ...
    .slice(0,5);
  glossaryElement.recentGlossary.innerHTML = recent.map(t => `<button class="recent-term" type="button" data-recent-id="${escapeHtml(t.id)}">${escapeHtml(t.name)}</button>`).join('') || '<span class="muted">まだ登録されていません。</span>';
